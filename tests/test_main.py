...
class TestMainNotConfigured:
    def test_exits_when_not_configured(self, capsys):
        with (
            patch("sys.argv", ["rathausrot"]),
            patch(
                "rathausrot.config_manager.ConfigManager.is_configured",
                return_value=False,
            ),
...
class TestMainSetup:
    def test_setup_flag(self):
        with (
            patch("sys.argv", ["rathausrot", "--setup"]),
            patch("rathausrot.config_manager.ConfigManager.get", return_value="INFO"),
            patch("rathausrot.utils.setup_logging"),
            patch("signal.signal"),
...
class TestMainTest:
    def test_test_flag(self, capsys):
        with (
            patch("sys.argv", ["rathausrot", "--test"]),
            patch("rathausrot.config_manager.ConfigManager.get", return_value="INFO"),
            patch(
                "rathausrot.config_manager.ConfigManager.is_configured",
                return_value=True,
            ...
            patch(
                "rathausrot.config_manager.ConfigManager.load",
                return_value={"matrix": {}},
            ),
            patch("rathausrot.utils.setup_logging"),
            patch("signal.signal"),
...
class TestMainRun:
    ...
    def test_run_now_flag(self):
        with (
            patch("sys.argv", ["rathausrot", "--run-now"]),
            patch("rathausrot.config_manager.ConfigManager.get", return_value="INFO"),
            patch(
                "rathausrot.config_manager.ConfigManager.is_configured",
                return_value=True,
